...
- [x] 1.1 Type checking passes with 0 errors: `npx astro check` — ffae327
- [x] 1.2 Unit tests pass: `npm run test` — ffae327
- [x] 1.3 Linting passes: `npm run lint` — ffae327
- [x] 1.4 Build succeeds: `npm run build` — ffae327

#### Manual

- [x] 1.5 Manually hit `GET /api/v1/tasks/` (or a task by id) in dev to confirm normal task retrieval still works unchanged. — ffae327
- [x] 1.6 Manually hit `PATCH /api/v1/tasks/<id>` with a `last_done_date` field to confirm the update still persists the formatted date correctly. — ffae327

### Phase 2: Wire astro check into CI

#### Automated

- [x] 2.1 New script runs cleanly and reports 0 errors: `npm run check` — 7fddf21
- [x] 2.2 Full CI job succeeds locally in sequence: `npx astro sync && npm run check && npm run lint && npm run test && npm run build` — 7fddf21

#### Manual

- [x] 2.3 Push the branch and confirm the GitHub Actions `ci` job's new "Type check" step appears and passes. — d8b29ea
