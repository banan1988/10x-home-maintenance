...
- [x] **Cookie check** — initially found a gap (PKCE `code-verifier`/session cookies missing `Secure`/`HttpOnly`, `@supabase/ssr`'s default options passed straight through with no override in `src/lib/supabase.ts`'s `setAll`). **Fixed and deployed 2026-08-24** in [PR #6](https://github.com/banan1988/10x-home-maintenance/pull/6) (`fix(auth): force Secure and HttpOnly on Supabase session cookies`, commit `fd3f522`, deployed as version `1157d509`) — `setAll` now merges `{ httpOnly: true, secure: true }` into every cookie's options. Verified live: `Set-Cookie` headers now show `HttpOnly; Secure; SameSite=Lax`. Covered by a new unit test (`src/lib/supabase.test.ts`, TDD red→green).
...
- **Risk register outcomes**:
  ...
  - **New finding, now resolved**: the Supabase session/PKCE cookies shipped without `Secure`/`HttpOnly` attributes (see Phase 5) — fixed in `src/lib/supabase.ts` and deployed 2026-08-24 via PR #6.
