...
- **Tailwind class merging**: use the `cn()` helper from `@/lib/utils` (clsx + tailwind-merge) for conditional/merged class names in `.tsx` components. Do not concatenate class strings manually. In `.astro` files, use Astro's native `class:list` directive instead — this is the established convention across every `.astro` component in the repo (e.g. `Banner.astro`, `Header.astro`).
...
1. **Matcher** — a filter deciding whether this hook runs (tool name like `Write`/`Edit`, file type, or a name pattern).
1. **Handler** — the action that runs, usually a shell command.
1. **Signal** — the result returns to the tool. The exit code says pass/fail; stdout can flow into the agent's context as feedback.
...
> > > > > > > 4ccb045 (chore(m3l3): init and get skills from m3l3)
